...
async function main() {
  ...
  try {
    console.log(
      "🔧 Creating Radix Agent (no auto-funding, user-driven approach)..."
    );
    ...
    console.log(
      "--- Test 2: Fund Wallet via Agent (Works Regardless of Balance!) ---"
    );
    console.log('Query: "Fund my wallet with testnet XRD"');
    console.log(
      "💡 Note: This will work even if wallet already has sufficient funds"
    );
    ...
    console.log("--- Test 5: Show All Assets ---");
    console.log('Query: "Show me all my token balances and assets"');
    const allAssets = await agent.run(
      "Show me all my token balances and assets"
    );
    ...
    const epochInfo = await agent.run(
      "What is the current epoch of the Radix network?"
    );
